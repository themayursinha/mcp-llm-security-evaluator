...
class ConfigValidator:
    ...
    def validate(provider: str = "auto") -> Tuple[bool, Optional[str]]:
        ...
        if Config.REPORT_FORMAT not in ["json", "html", "both"]:
            return False, f"REPORT_FORMAT must be 'json', 'html', or 'both', got '{Config.REPORT_FORMAT}'"
        
        if not (0 <= Config.SECURITY_THRESHOLD <= 100):
            return False, f"SECURITY_THRESHOLD must be between 0 and 100, got {Config.SECURITY_THRESHOLD}"
        
        if Config.LOG_LEVEL not in ["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"]:
            return False, f"LOG_LEVEL must be one of: DEBUG, INFO, WARNING, ERROR, CRITICAL, got '{Config.LOG_LEVEL}'"
