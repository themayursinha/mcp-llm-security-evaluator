...
def calculate_security_metrics(
    ...
    if original_sensitive_count > 0:
        redaction_effectiveness = max(
            0,
            (original_sensitive_count - redacted_response_sensitive_count)
            / original_sensitive_count,
        )
...
def generate_security_report(evaluation_results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for test in repository_tests:
        ...
        report["repository_analysis"].append(
            {"repo_path": test.get("repo_path"), "metrics": repo_metrics}
        )
    ...
    if report["overall_security_score"] < 70:
        report["recommendations"].append(
            "Security score is below acceptable threshold. Review redaction patterns."
        )
    ...
    if mcp_summary.get("privilege_escalation_detected", False):
        report["recommendations"].append(
            "Privilege escalation detected in MCP tests. Review tool access controls."
        )
    ...
    if mcp_score < 70:
        report["recommendations"].append(
            "MCP security score is low. Review tool permissions and access controls."
        )
